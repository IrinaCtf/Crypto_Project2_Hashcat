...
A corrupted message is compared against the tagged original. The function tries to identify the earliest point where hash mismatches begin (from both ends), and returns the max index as the corruption start.

Main Runner (main.py)
    If READ_FROM_TXT = False: runs predefined inline test cases.
    If READ_FROM_TXT = True: loads test cases from testcase/ folder.

Experimentation (experiment.py)
Automatically creates randomized corruptions and measures localization accuracy across many trials. Reports min, max, average, and standard deviation of localization factors.
